...
def generate_keys(message):
    ...
    key_length = message_length * 8  # Assuming 8 bits per character
...
def encrypt_message(modified_message):
    ...
    for char in modified_message:
        ...
        encrypted_char = chr(ascii_value + 2)  # Add 2 to the ASCII value
...
def decrypt_message(encrypted_message):
    ...
    for char in encrypted_message:
        ascii_value = ord(char)
        decrypted_char = chr(ascii_value - 2)  # Subtract 2 from the ASCII value
